...
grep -E "^test |^test result|^A[0-9] error" "$R/.spike-S-A.log" | sed -E 's/ finished in [0-9.]+s//' | sed -E 's/account_id: [1-9A-HJ-NP-Za-km-z]{32,44}/account_id: <pda>/g' | sort > "$EV/S-A.run.txt"
if [ "$rc" -eq 0 ]; then
