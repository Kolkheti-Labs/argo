...
//! `-- --nocapture`.
#![allow(clippy::unwrap_used, clippy::expect_used, clippy::indexing_slicing, clippy::as_conversions, reason = "spike")]
...
fn try_execute(elf: &[u8], program_id: ProgramId, caller: Option<ProgramId>, pre: Vec<AccountWithMetadata>, ix: impl serde::Serialize) -> Result<Run, String> {
    ...
    let info = default_executor().execute(env, elf).map_err(|e| e.to_string())?;
    Ok(Run { padded: info.segments.iter().map(|s| u64::from(s.cycles)).sum(), segments: info.segments.len() })
}
fn execute(elf: &[u8], program_id: ProgramId, caller: Option<ProgramId>, pre: Vec<AccountWithMetadata>, ix: impl serde::Serialize) -> Run {
...
fn state_account(ops: u64, pad_len: usize) -> AccountWithMetadata {
    let st = VaultState { ops, internal_hits: 0, vault: vault_id(&spike_id()), pad: vec![7u8; pad_len] };
    AccountWithMetadata {
        account: Account {
...
fn row(name: &str, r: &Run) {
    eprintln!("{name:<44} {:>12} cycles  {:>2} seg  {:>5.1}% of 32M", r.padded, r.segments, 100.0 * r.padded as f64 / BUDGET as f64);
}
...
fn s_d_cycle_table() {
    ...
    let argo_leg = execute(elf, spike_id(), None, vec![state_account(1, 0), holding(user, 1_000_000, true), holding(vault_id(&spike_id()), 0, false)], Instruction::PayIn { amount: 1_000 });
    row("D1 PayIn: Argo leg", &argo_leg);
    let token_leg = execute(programs::token().elf(), token_id(), Some(spike_id()), vec![holding(user, 1_000_000, true), holding(vault_id(&spike_id()), 0, false)], token_core::Instruction::Transfer { amount_to_transfer: 1_000 });
    row("D1 token Transfer leg", &token_leg);

    // D2 liquidate-shaped: Argo leg emitting 2 chained calls (transfer + self-call).
    let pay_out = execute(elf, spike_id(), None, vec![state_account(1, 0), holding(vault_id(&spike_id()), 500_000, false), holding(recipient, 0, false)], Instruction::PayOut { amount: 1_000, then_fail: false, wrong_seed: false });
    row("D2 PayOut: Argo leg (2 chained calls)", &pay_out);
    let internal = execute(elf, spike_id(), Some(spike_id()), vec![state_account(2, 0)], Instruction::Internal { must_fail: false });
    row("D2 Internal self-call leg", &internal);

    // D2b math: cycles per iteration of accrual+health+LIF-shaped arithmetic.
    let base = execute(elf, spike_id(), None, vec![state_account(1, 0)], Instruction::Stress { iters: 0, pad: 0 });
    row("D2b Stress iters=0 (baseline)", &base);
    let mut per_iter = 0f64;
    for iters in [100u32, 1_000, 3_000, 6_000] {
        match try_execute(elf, spike_id(), None, vec![state_account(1, 0)], Instruction::Stress { iters, pad: 0 }) {
            Ok(r) => {
                row(&format!("D2b Stress iters={iters}"), &r);
                per_iter = (r.padded.saturating_sub(base.padded)) as f64 / f64::from(iters);
            }
            Err(e) => eprintln!("{:<44} over budget: {e}", format!("D2b Stress iters={iters}")),
        }
    }
    ...
    for pad in [1_024u32, 10_240, 51_200] {
        match try_execute(elf, spike_id(), None, vec![state_account(1, pad as usize)], Instruction::Stress { iters: 0, pad }) {
            Ok(r) => { row(&format!("D3 Stress pad={pad}B"), &r); last = r.padded; }
    ...
    eprintln!("{:<44} {liquidate_chain:>12} cycles (sum of legs, future tx-wide budget)", "D2 liquidate-shaped chain total");

    let verdict = if pay_out.padded < BUDGET / 2 && liquidate_chain < BUDGET { "GO" } else if pay_out.padded < BUDGET { "PARTIAL" } else { "NO-GO" };
