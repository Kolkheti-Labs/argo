...
cargo test --release -p spike_integration_tests --test spike_ab -- b 2>&1 | tee "$LOG"
if grep -q 'test result: ok' "$LOG" && ! grep -q 'FAILED' "$LOG"; then
  echo "VERDICT S-B: GO -- anyone credits a claimed vault; only the program debits it (by seed); credit-before-claim rejected, claim-first ordering confirmed"
else
