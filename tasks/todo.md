...
- [ ] T7 Evidence model: `harness/capture.sh` snapshots raw JSON-RPC responses per claim into `evidence/`, regenerable by one command (testnet wipes orphan ids)
...
- [ ] T13 State-layout spec `docs/spec/state-layout.md`: PDA seeds, account byte layouts, touched-set per instruction, sharding argument for P2 (D5)
- [x] T14 Verdicts (all five written): `docs/m0/verdicts.md`, one go/no-go per spike; any no-go carries the M1–M3 design change
- [~] T15 Fresh-clone verification: interim = fresh toolchain + fresh clone on hetzner2 volume (harness/clean-host.sh); true fresh box pending Davit (hetzner2 volume or a fresh box): `git clone && ./harness/localnet.sh && cargo test` green from nothing. Record host, commit, log
- [ ] T16 adversarial-verify pass on the whole milestone before calling it done
- [ ] T17 Milestone report for Logos: link every D1–D6 to its artifact and its verification log
